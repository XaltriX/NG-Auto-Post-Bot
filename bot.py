...
async def add_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if channel_id not in channels[user_id]:
            ...
            await message.reply_text("ℹ️ This channel is already in your list!")
...
async def thumbnail_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.photo:
        ...
        await message.reply_text("⚠️ Please send a valid thumbnail (photo, video, or GIF).")
        return THUMBNAIL
    ...
    await message.reply_text("👍 Great! Now please send me the video link. 🔗")
    return VIDEO_LINK
...
async def post_to_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(status_message, reply_markup=reply_markup)
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        ...
        await update.callback_query.message.edit_text("❌ Operation cancelled. Send /start to begin again.")
    else:
...
def main():
    ...
    application = Application.builder().token('7008031473:AAHIEimSbG_ryMPvJa57VbXaOp5wEF9RNtk').build()
